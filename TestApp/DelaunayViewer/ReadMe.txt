================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : DelaunayViewer プロジェクトの概要
===============================================================================

この DelaunayViewer アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、DelaunayViewer アプリケーションを構成する各ファイルの内容の概要が含まれています。

DelaunayViewer.vcxproj
...
DelaunayViewer.vcxproj.filters
...
DelaunayViewer.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CDelaunayViewerApp アプリケーション クラスを宣言します。

DelaunayViewer.cpp
    これはメインのアプリケーション ソース ファイルで、CDelaunayViewerApp アプリケーション クラスが含まれています。
...
res\DelaunayViewer.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル DelaunayViewer.rc にインクルードされます。

res\DelaunayViewer.rc2
...
DelaunayViewerDlg.h, DelaunayViewerDlg.cpp - ダイアログ
    これらのファイルには、CDelaunayViewerDlg クラスが含まれます。このクラスはアプリケーションのメイン ダイアログの動作を定義します。ダイアログのテンプレートは、Microsoft Visual C++ で編集できる DelaunayViewer.rc に含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、DelaunayViewer.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

DelaunayViewer.manifest
